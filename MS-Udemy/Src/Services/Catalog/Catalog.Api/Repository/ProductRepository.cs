...
namespace Catalog.Api.Repository
{
    public class ProductRepository : IProductRepository
    {
        ...
        {
            var filterBuilder = Builders<Product>.Filter.Eq(p => p.Category, category);
            return await _context.Products.Find(filterBuilder).ToListAsync();
        }


        public async Task<IEnumerable<Product>> GetProductsByNameAsync(string name)
        {
            var filterbuilder = Builders<Product>.Filter.Eq(p => p.Name, name);
            return await _context.Products.Find(filterbuilder).ToListAsync();
        }
